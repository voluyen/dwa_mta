...
class Distiller(nn.Module):
    def __init__(self, args, device):
        ...
        # Load original model for self-correction KL divergence computation
        if hasattr(args, 'original_model_path') and args.original_model_path is not None:
            self.original_model = self.load_original_model()
            log_rank(f"Loaded original model from {args.original_model_path}")
        else:
    ...
    def load_tokenizer(self, model_type, path):
        ...
        if model_type in ["gpt2", "opt", "llama", "gptj", "llama2", "llama3", "mistral", "tinyllama", "minicpm", "gemma2"]:
    ...
    def load_original_model(self):
        """Load original model for computing p(y1|x) in self-correction KL divergence"""
        log_rank("Loading original model for KL divergence computation...")
        config = AutoConfig.from_pretrained(self.args.original_model_path, trust_remote_code=True)
        config.is_model_parallel = False
        ...
        log_rank(' > number of parameters of the original model: {:,}'.format(
            sum([p.nelement() for p in model.parameters()])
        ))
